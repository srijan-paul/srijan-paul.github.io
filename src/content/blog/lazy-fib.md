Functional languages are assumed to be inferior when tackling problems
that are best solved with dynamic programming.
This belief is largely based on the absence of state and mutation from
pure languages.
It is true that DP often requires a global state (the memoization table),
and continuous updates to the said state.

With lazy evaluation however, a declarative solution's elegance
far supercedes imperative solutions.
...
Most imperative solutions to calculating the Nth fibonacci number would
involve a table of some sort, where each number `i` is mapped to `Fib_i`.
The first two items in the table are mapped to 0 and 1 (or 1 and 2), and the prgoram
then iterates over the size of the table,
setting `fib[i]` to `fib[i - 1] + fib[i - 2]`.
...
Over the remainder of this post, we'll go over two solutions to this problem.
...
The rest of the operations in the code are stateless and
referentially transparent.
In order to implement memoization in a functional manner,
we're going to leverage two features of Haskell:
...
Haskell is a lazyily evaluated language, wherein no value
is evaluated until its needed.
...
When we call `print`, the expression `a + b` *must* be evaluated
so that it can be written to stdout.
Before the runtime encounters the call to `print` however,
`a`, `b` and `c` remain "frozen".

Since we made the assumption that `get_a` and `get_b` require 1 and 2 seconds
to compute respectively, `a + b` will block the thread for a total of 3 seconds
whereas `get_c` will never be visited.
...
If you store it in a variable however, the REPL won't evluate it right away:

```hs
Prelude> xs = [1, 2 ..]
```

We can use `xs` as if it were a regular linked list:
...
This is purely to build an intuition for the subject,
and is not idiomatic Python code.

We start by defining `make_list` - a function that returns an infinite list.
Later, we're going to define `rest` and `index_list`,
to mirror haskell's `tail` and `(!!)` functions
...
This is to prevent the interpreter from eagerly evaluating the values stored inside.
The first elemet is simply the head of the list.
...
It looks gnarly at first, but becomes easier to grasp
once you realize that we're really
just using `lambda` to "freeze" a value.
The interpreter won't evalute a frozen value
unless we "thaw" it using the call operator - `()`.

At any given point in time, our list always contains two items:
...
With that, we have an infinite list very similar to the one we wrote in Haskell.
